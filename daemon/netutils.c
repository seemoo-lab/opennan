...
int neighbor_add(int ifindex, const struct ether_addr *eth, const struct in6_addr *in6)
{
	...
	struct nl_addr *nl_ipaddr = nl_addr_build(AF_INET6, (void *)in6, sizeof(struct in6_addr));
	if (!nl_ipaddr)
	{
		log_error("Could not allocate netlink message");
		return -ENOMEM;
	...
	struct nl_addr *nl_llcaddr = nl_addr_build(AF_LLC, (void *)eth, sizeof(struct ether_addr));
	if (!nl_llcaddr)
	{
		log_error("Could not allocate netlink message");
		return -ENOMEM;
	...
	int err = rtnl_neigh_add(nlroute_state.socket, neigh, NLM_F_CREATE);
	if (err < 0)
	{
...
int neighbor_remove(int ifindex, const struct in6_addr *in6)
{
	...
	int err = rtnl_neigh_delete(nlroute_state.socket, neigh, 0);
	if (err < 0)
	{
...
int set_channel(int ifindex, int channel /* TODO int ctrl_freq , int bw, int cntr_freq */)
{
	...
	{
		log_error("Could not gen message");
		err = -ENOBUFS;
		goto out;
	}
...
out:
	if (m)
		nlmsg_free(m);
	if (err < 0)
		log_error("error while setting channel: %s", nl_geterror(err));
	return err;
}

int link_updown(int ifindex, bool up)
{
	int err;
...
{
	return link_updown(ifindex, true);
}

int link_down(int ifindex)
{
	return link_updown(ifindex, false);
}
